...
class Todo {
  ...
  static async where(val, key) {
    const snapshot = await todoRefs.orderByChild(key).equalTo(val).once('value');
    const records = _.pipe([
      _.toPairs,
      _.map(keyValuePair => ({ id: keyValuePair[0], ...keyValuePair[1] })),
      _.values,
    ])(snapshot.val());
    // _.mapValue((v, k) => ({ id: k, ...v }), snapshot.val());
